...
def run_ablation_study(pretrained_path=None, config=None, dataset_name='default'):
    ...
    Args:
        ...
        dataset_name: 数据集名称 ('default', 'taiwan', 'consumer')
    """
    import pandas as pd
    import tempfile
    ...
    if dataset_name == 'taiwan':
        ...
        if os.path.exists(data_file):
            ...
            if rename_map:
                ...
                df.to_excel(temp_file.name, index=False)
                data_file = temp_file.name
                print(f"  Consumer数据集列名适配: {rename_map}")
    ...
    # 保存结果
    result_file = f'ablation_results_{dataset_name}.json'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='telecom',
                       choices=['telecom', 'taiwan', 'consumer', 'all'],
                       help='数据集选择')
    ...
    if args.dataset == 'taiwan' or args.dataset == 'all':
        print("\n" + "=" * 70)
        print("台湾餐厅数据集消融实验 (三模态, 加权损失)")
        print("=" * 70)
        from config import get_taiwan_restaurant_config
        config_tw = get_taiwan_restaurant_config()
        run_ablation_study(pretrained_path=None, config=config_tw, dataset_name='taiwan')

    if args.dataset == 'consumer' or args.dataset == 'all':
        print("\n" + "=" * 70)
        print("Consumer Complaint数据集消融实验 (双模态)")
        print("=" * 70)
        from config import get_consumer_complaint_config
        config_cc = get_consumer_complaint_config()
        run_ablation_study(pretrained_path=None, config=config_cc, dataset_name='consumer')