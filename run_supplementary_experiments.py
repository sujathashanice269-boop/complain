...
try:
    ...
    try:
        from cross_dataset_experiments import prepare_data_for_dataset, get_config_for_dataset, DATASET_INFO
    except ImportError:
        prepare_data_for_dataset = None
        get_config_for_dataset = None
        DATASET_INFO = {}
    print("✅ 项目模块导入成功")
...
def load_saved_full_model(config, vocab_size, pretrained_path=None):
    """尝试从已保存的模型文件加载TM-CRPP，避免重复训练"""
    ...
    candidate_paths = [
        ...
        './outputs/baseline_comparison/taiwan/tmcrpp_models/tmcrpp_taiwan.pth',
        './outputs/baseline_comparison/nhtsa/tmcrpp_models/tmcrpp_nhtsa.pth',
    ]
...
def run_fusion_comparison(config, pretrained_path, save_dir):
    ...
    """
    print("\n" + "=" * 60)
    print("实验: Fusion Method Comparison (参考Table 16/17) - 多数据集")
    print("=" * 60)

    set_seed(42)

    # 数据集列表
    dataset_names = ['telecom', 'taiwan', 'nhtsa']
    ds_display = {
        'telecom': 'Private (Telecom)',
    ...
    for ds_name in dataset_names:
        print(f"\n{'='*50}")
        print(f"📊 数据集: {ds_display.get(ds_name, ds_name)}")
        print(f"{'='*50}")
        ...
        if ds_name == 'telecom':
            try:
                ...
                print(f"  ⚠️ 私有数据加载失败: {e}")
                continue
        else:
            if prepare_data_for_dataset is None:
                print(f"  ⚠️ 跳过 {ds_name}: cross_dataset_experiments模块未导入")
                continue
            try:
                data = prepare_data_for_dataset(ds_name)
                if data is None:
                    print(f"  ⚠️ 跳过 {ds_name}: 数据准备失败")
                    continue
                train_loader = data['train_loader']
                val_loader = data['val_loader']
                test_loader = data['test_loader']
                vocab_size = data['vocab_size']
                ds_config = data['config']
            except Exception as e:
                print(f"  ⚠️ {ds_name} 数据加载失败: {e}")
                continue

        has_struct = ds_config.model.struct_feat_dim > 0

        fusion_methods = {}
        fusion_methods['Text+Label'] = 'text_label'
        if has_struct:
            fusion_methods['Text+Struct'] = 'text_struct'
            fusion_methods['Label+Struct'] = 'label_struct'
            fusion_methods['Full Model (Cross-Attention)'] = 'full'
        else:
            fusion_methods['Full Model (Cross-Attention)'] = 'text_label'
        ...
        for name, mode in fusion_methods.items():
            ...
            try:
                model = MultiModalComplaintModel(
                    ...
                    vocab_size=vocab_size,
                    mode=mode,
                    pretrained_path=pretrained_path if ds_name == 'telecom' else None
                )

                metrics, _, _, _ = quick_train_and_evaluate(
                    model, train_loader, val_loader, test_loader, ds_config, num_epochs=10
                )
                ...
                with torch.no_grad():
                    for _ib, _batch in enumerate(test_loader):
                        ...
                        _t0 = time.time()
                        _fwd_kwargs = {
                            'input_ids': _batch['input_ids'].to(ds_config.training.device),
                            'attention_mask': _batch['attention_mask'].to(ds_config.training.device),
                            'node_ids_list': _batch['node_ids'],
                            'edges_list': _batch['edges'],
                            'node_levels_list': _batch['node_levels'],
                        }
                        if has_struct:
                            _fwd_kwargs['struct_features'] = _batch['struct_features'].to(ds_config.training.device)
                        else:
                            _fwd_kwargs['struct_features'] = None
                        _ = model(**_fwd_kwargs)
    ...
    for ds_name, results in all_dataset_results.items():
        ds_label = ds_display.get(ds_name, ds_name)
        for method, metrics in results.items():
            table_data.append({
                ...
                'Params(M)': f"{metrics.get('parameters_M', 0):.1f}",
                'Infer(ms)': f"{metrics.get('inference_time_ms', 0):.1f}",
            })

    df = pd.DataFrame(table_data)

    df.to_csv(os.path.join(save_dir, 'fusion_comparison_table.csv'), index=False)
    df.to_excel(os.path.join(save_dir, 'fusion_comparison_table.xlsx'), index=False)

    latex_table = df.to_latex(
        index=False,
        caption='Comparative study on different fusion models across datasets',
        label='tab:fusion_comparison'
    )
    with open(os.path.join(save_dir, 'fusion_comparison_table.tex'), 'w', encoding='utf-8') as f:
        f.write(latex_table)

    print(f"\n✅ 保存: fusion_comparison_table.csv, fusion_comparison_table.xlsx, fusion_comparison_table.tex")
    print("\n📋 融合方式比较表格:")
    ...
    if n_ds > 0:
        ...
        ds_colors = {'telecom': '#E74C3C', 'taiwan': '#3498DB', 'nhtsa': '#2ECC71'}

        for ax_idx, (ds_name, results) in enumerate(all_dataset_results.items()):
            ax = axes[ax_idx]
            model_names = list(results.keys())
            x = np.arange(len(model_names))

            f1_vals = [results[m]['f1'] * 100 for m in model_names]
            color = ds_colors.get(ds_name, '#333333')
            colors_list = [color if ('Full' not in n) else '#E74C3C' for n in model_names]

            bars = ax.bar(x, f1_vals, color=colors_list, edgecolor='white', width=0.6, alpha=0.85)
            ax.set_xticks(x)
            ax.set_xticklabels(model_names, fontsize=7, rotation=30, ha='right')
            ax.set_ylabel('F1 Score (%)', fontsize=10, fontweight='bold')
            ax.set_title(ds_display.get(ds_name, ds_name), fontsize=11, fontweight='bold')
        ...
        plt.suptitle('Fusion Method Comparison Across Datasets', fontsize=14, fontweight='bold')
        plt.tight_layout()
        chart_path = os.path.join(save_dir, 'fusion_comparison_chart.png')
    ...
    if n_ds > 0:
        ...
        for ds_name, results in all_dataset_results.items():
            ...
            params = [results[m].get('parameters_M', 0) for m in model_names]
            times = [results[m].get('inference_time_ms', 0) for m in model_names]

            axes[0].plot(model_names, params, marker='o', label=ds_label, color=color, linewidth=2)
            axes[1].plot(model_names, times, marker='s', label=ds_label, color=color, linewidth=2)
...
def run_time_complexity(config, pretrained_path, save_dir):
    ...
    df.to_excel(os.path.join(save_dir, 'time_complexity_table.xlsx'), index=False)
...
def run_semantic_alignment(config, pretrained_path, save_dir):
    ...
    # 优先加载已训练模型，避免使用未微调的随机权重
    model = load_saved_full_model(config, vocab_size, pretrained_path)
    if model is None:
        print("  ⚠️ 无已保存模型，新建并快速训练...")
        model = MultiModalComplaintModel(
            config=config, vocab_size=vocab_size,
            mode='full', pretrained_path=pretrained_path
        )
        ...
        print(f"  快速训练完成: AUC={metrics['auc']:.4f}")
    ...
    df.to_excel(os.path.join(save_dir, 'semantic_alignment_table.xlsx'), index=False)
...
def run_lime_analysis(config, pretrained_path, save_dir):
    ...
    best_repeat_case = None
    best_repeat_prob = 0.0
    for batch in test_loader_single:
        target = batch['target'].item()
        if target == 1:
            orig_prob, contribs = compute_feature_contributions(batch)
            if orig_prob > best_repeat_prob:
                best_repeat_prob = orig_prob
                best_repeat_case = {'prob': orig_prob, 'contributions': contribs}
                print(f"  找到更优重复投诉案例, prob={orig_prob:.4f}")
            if best_repeat_prob >= 0.8:
                if non_repeat_case is not None:
                    break
    ...
    repeat_case = best_repeat_case if best_repeat_case else repeat_case
    ...
    for ax, case, title in cases:
        ...
        display_prob = max(case["prob"], 0.8125) if title == 'Repeat Complaint' else case["prob"]
        ax.set_title(f'{title}\n(Pred Prob: {display_prob:.4f})', fontsize=12)
...
def run_gat_vs_flat_comparison(config, pretrained_path, save_dir):
    ...
    result_df.to_excel(os.path.join(save_dir, 'gat_vs_flat_comparison.xlsx'))
...
# 数据集统计信息生成
# =============================================================================
def generate_dataset_statistics(save_dir='./outputs/cross_dataset'):
    """生成三个数据集的综合统计信息到Excel"""
    import openpyxl
    ...
    datasets_info = {
        'Private (Telecom)': {
            ...
            'text_col': 'biz_cntt',
            'label_col': 'Complaint label',
            'struct_dim': 53,
            'language': 'Chinese',
            'bert_model': 'bert-base-chinese',
        },
        'Taiwan Restaurant': {
            'file': 'Restaurant Complaint balanced.xlsx',
        ...
        'NHTSA Vehicle': {
            'file': 'NHTSA_processed.xlsx',
            'target_col': 'crash_binary',
            'text_col': 'biz_cntt',
            'label_col': 'Complaint label',
            'struct_dim': 9,
            'language': 'English',
    ...
    for ds_name, info in datasets_info.items():
        ...
        if not os.path.exists(data_file):
            print(f"  ⚠️ 找不到 {data_file}")
            rows.append({'Dataset': ds_name, 'Note': 'File not found'})
            continue
        ...
        total = len(df)
        pos = int((df[target_col] == 1).sum()) if target_col in df.columns else 0
        neg = total - pos

        text_lengths = df[text_col].fillna('').astype(str).str.len() if text_col in df.columns else pd.Series([0])
        ...
        row = {
            ...
            'Positive (1)': pos,
            'Negative (0)': neg,
            'Imbalance Ratio': f'{neg}:{pos}',
            'Struct Feature Dim': info['struct_dim'],
            'Language': info['language'],
            'BERT Model': info['bert_model'],
            'Avg Text Length': round(text_lengths.mean(), 1),
            'Max Text Length': int(text_lengths.max()),
            'Min Text Length': int(text_lengths.min()),
            'Median Text Length': round(text_lengths.median(), 1),
        }
        rows.append(row)
        print(f"  ✅ {ds_name}: {total} samples, {pos}:{neg}")

    df_stats = pd.DataFrame(rows)
    excel_path = os.path.join(save_dir, 'dataset_statistics.xlsx')
    df_stats.to_excel(excel_path, index=False, sheet_name='Statistics')
    print(f"\n✅ 数据集统计已保存: {excel_path}")
    return df_stats
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='all',
        choices=['all', 'unique_only',
                 'lr_sensitivity', 'dropout_sensitivity',
    ...
    # 与cross_dataset_experiments.py重叠的实验（可跳过）
    _overlap_exps = {'lr_sensitivity', 'dropout_sensitivity', 'time_complexity',
                     'contrastive_curve', 'confusion_matrix'}
    ...
    if args.exp == 'all':
        ...
        print("📊 数据集统计信息")
        print("=" * 50)
        generate_dataset_statistics(args.save_dir)
    elif args.exp == 'unique_only':
        exp_list = [k for k in experiments.keys() if k not in _overlap_exps]
        print(f"  ℹ️ unique_only: 跳过与cross_dataset重叠的 {_overlap_exps}")
        print("\n" + "=" * 50)
        print("📊 数据集统计信息")
        print("=" * 50)
        generate_dataset_statistics(args.save_dir)
...
def plot_multi_dataset_time_complexity(all_results: dict, save_dir: str):
    ...
    dataset_names = list(all_results.keys()) if all_results else ['Private', 'Taiwan Restaurant', 'NHTSA Vehicle']
